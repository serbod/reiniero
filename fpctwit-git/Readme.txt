...
It is written for FreePascal but meant to be usable in Delphi as well. It currently uses the Synapse network library (included) but can be adapted for other network libraries.
...
It supports OAuthv1 secure authentication and TLS encryption. The library uses modified fpJSON units to allow getting UTF8 data out of the JSON data with Tweets returned from Twitter. For newer FPC versions, it will use the native fpJSON units as those support UTF8.
...
The applications and libraries are distributed under a very liberal license (my code under MIT license; see Synapse and fpjson units for their licenses) which allows commercial use as well as interoperability with GPL programs.
